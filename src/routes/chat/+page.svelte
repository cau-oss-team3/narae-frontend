...
<script>
    ...
    $: if(chat_history && element) {
        if (chat_history[chat_history.length-1].type > 3 || chat_history[chat_history.length-1].candidates.length > 0) {
    ...
    async function sendMessage() {
        const data = { // 채팅창에 입력한 데이터임 이거 사용하면됨
            type: 0,
...
<div id="chat_container" class="container-fluid" style="height: 75vh;">
    <div bind:this={element} style="height: 100%; overflow-y: scroll; margin-top: 20px; padding-bottom: 200px; padding-left: 10px; padding-right: 10px;" >
            ...
            {#if message.visibility}
                {#if message.type == 0}
                    <Card style="margin-left: 20%; margin-bottom: 20px; text-align: end;">
                        ...
                        <CardFooter>{new Intl.DateTimeFormat('ko-KR', { year: 'numeric', month: 'numeric', day: 'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric' }).format(message.timestamp)}</CardFooter>
                    </Card>
                ...
                {:else if message.type == 5}
                    <Card style="margin-right: 20%; margin-bottom: 20px;">
                        ...
                        <CardBody>
                            ...
                            <Form {validated} action="javascript:void(0);" on:submit={(e) => postActionResult(index, e.submitter.value, message.candidates[0], input_temp_data) } method="post" >
                                <Row>
                                    <Col>
                                        <FormGroup floating label="왜 성공 / 실패 / 보류 했는지 짧게 작성해주세요." class="form-outline mb-4">
                                            <Input type="text" name="input_chat" id="input_chat" required bind:value={input_temp_data}/>
                                        </FormGroup>
                                    </Col>
                                </Row>
                                <Row>
    ...
    <Form {validated} action="javascript:void(0);" on:submit={(e) => sendMessage() } method="post" id="chatForm">
        <Container fluid style="display: {inputChatDisplayValue};">
            <Row>
                <Col>
                    <FormGroup floating label="여기에 내용을 입력하세요." class="form-outline mb-4">
                        <Input type="text" name="input_chat" id="input_chat" required bind:value={input_chat_data}/>
                    </FormGroup>
                </Col>
                <Col xxs="2" sm="2">
                    <Button
